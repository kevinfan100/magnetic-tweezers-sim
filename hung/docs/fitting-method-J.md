...
- 擬合腳本：`hung/analysis/fit/fit_J.m`
- 出圖腳本：`hung/analysis/plot/plot_J_quiver.m`（% 誤差）、`hung/analysis/plot/plot_J_rmse.m`（RMSE）
- 依賴：
  - `hung/analysis/core/mt_constants.m` — 幾何常數
  - `hung/analysis/core/import_ansys_data.m` — FEM 數據讀取
- FEM 數據：`hung/results/coil[1-6]/filleted/coilN_{coord,bfield}_wp.dat`
