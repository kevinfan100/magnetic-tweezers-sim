...
## Description
Same parts as `hung/IGES/`, but with IGES unit flag corrected from 6 (mm) to 1 (inches).
This ensures reliable conversion when saving as STEP in SolidWorks or other CAD software.

## Difference from `hung/IGES/`
- `hung/IGES/` — raw ANSYS export, unit flag = 6 (mm), values in inches. SolidWorks opens correctly but STEP conversion may fail.
- `hung/IGES_converted/` — unit flag fixed to 1 (inches). SolidWorks opens correctly AND STEP conversion preserves dimensions.
...
## Regenerating
When parts change, re-export from `hung/IGES/` and fix unit flag:
```bash
