...
MATLAB fitting results produced by scripts in `hung/analysis/`. Structure mirrors `hung/analysis/`:
...
Not in the active pipeline. Kept available; re-runnable from `hung/analysis/variants/` if needed.
...
All active files are regenerable by running the corresponding script in `hung/analysis/fit/`, assuming the upstream prerequisites exist (see pipeline order).

`charge_model_fit.mat` is the one exception — there is no Hung script that produces it yet. If it's ever deleted and needs to be rebuilt, a Hung method [A] script must be written (similar to `hexapole-long2016/analysis/fit_charge_model.m`).
