...
當工作涉及 `hung/` 目錄下的任何檔案時（建模、模擬、後處理、繪圖），**開始動手前必須依序讀取以下文件**：

1. `hung/docs/troubleshooting.md` — 已知陷阱，避免重蹈覆轍（最優先）
2. `hung/docs/hexapole-build-workflow.md` — 建模 + 模擬完整 SOP（步驟 1~7）
3. `hung/docs/hexapole-sphere-geometry.md` — 球面幾何、座標系、tip 座標
4. `hung/docs/pole-geometry.md` — pole 尺寸、傾斜角、正交驗證結果
5. `hung/docs/hexapole-simulation-reference.md` — 通用模擬規範（材料、元素、求解器）
